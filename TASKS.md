...
- [ ] Read `AGENTS.md`, this file, and any more-specific instruction files.
  - Acceptance: implementation notes acknowledge the Git and deployment prohibitions.

- [ ] Inspect the existing working tree without changing Git state.
  - Acceptance: preserve existing code and user edits; use only permitted read-only Git commands if Git inspection is needed.

- [ ] If the repository is empty, scaffold React, TypeScript, and Vite locally with npm.
...
- [ ] Establish scripts for `dev`, `lint`, `typecheck`, `test`, `test:watch`, and `build`.
  - Acceptance: every script is documented in `README.md` and runs locally.

- [ ] Create the feature-oriented source boundaries described in `AGENTS.md`.
...
- [ ] Define strict domain types for `Score`, `FolderScan`, reconciliation changes, preset, set, and generated performance.
...
- [ ] Create a versioned Dexie database and repository interfaces.
...
- [ ] Implement the initial IndexedDB schema and at least one migration-test harness.
...
- [ ] Implement score CRUD operations needed by the MVP.
...
- [ ] Persist and retrieve the most recently generated performance.
...
- [ ] Implement a reusable `.mscz` filename parser as a pure function.
...
- [ ] Convert a selected `FileList` into a metadata-only folder snapshot.
...
- [ ] Detect duplicate numeric score numbers within a snapshot.
  - Acceptance: every conflicting filename is reported and conflicts cannot overwrite one another.

- [ ] Build the **Import scores from this folder** control.
...
- [ ] Build the initial import preview.
...
- [ ] Implement reconciliation classification as a pure function.
...
- [ ] Add tests for configuration preservation.
...
- [ ] Build a mobile reconciliation preview.
...
- [ ] Implement explicit possible-renumbering resolution.
...
- [ ] Apply accepted reconciliation changes transactionally.
...
- [ ] Build score configuration editing.
...
- [ ] Build the pending-configuration queue.
...
- [ ] Make missing-score behavior safe and visible.
...
- [ ] Implement preset eligibility as pure functions.
...
- [ ] Implement a seedable pseudo-random source.
...
- [ ] Implement pre-generation feasibility validation.
...
- [ ] Implement the core performance generator.
...
- [ ] Implement the drums-intro soft rule.
...
- [ ] Add property-style generator tests across many deterministic seeds and pool shapes.
...
- [ ] Build the generated-performance view.
...
- [ ] Autosave the most recently generated performance.
...
- [ ] Provide a deliberate regenerate flow.
...
- [ ] Add a visible validation assertion in development/test builds.
...
- [ ] Add the web app manifest and local icon assets.
...
- [ ] Add a service worker and offline app-shell caching.
...
- [ ] Add iPhone installation guidance.
...
- [ ] Complete mobile accessibility and layout checks.
...
- [ ] Verify privacy behavior through code inspection and tests.
...
- [ ] Add a local Cloudflare Workers Static Assets configuration.
...
- [ ] Validate the Cloudflare configuration locally only.
...
- [ ] Document owner-performed deployment in `README.md`.
...
- [ ] Add/finish unit and component tests for every critical rule.
...
- [ ] Inspect the production bundle for accidental remote dependencies or secrets.
...
- [ ] Review the working-tree diff without staging or committing.
