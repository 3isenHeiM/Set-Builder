...
describe('settings backup', () => {
  it('exports a readable, versioned settings-only document sorted by score number', () => {
    const backup = createSettingsBackup([
      ...
      makeScore({ id: 'first', scoreNumber: 2, title: 'Africa', tags: ['80s'], hotness: 3, drumsIntro: true }),
  ...
  it('clamps old five-level backups when importing them', () => {
    const restored = parseSettingsBackupJson(JSON.stringify({
      format: SETTINGS_BACKUP_FORMAT,
      version: 1,
      exportedAt: 'now',
