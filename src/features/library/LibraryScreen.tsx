...
export function LibraryScreen({ scores, lastScan, onFolder, onSave, onEditingChange }: LibraryScreenProps) {
  ...
  if (scores.length === 0) {
    return (
      <section className="empty-state">
        ...
        <h1>Build tonight’s sets.</h1>
        <p>Use <strong>01 - Name.mscz</strong>.</p>
  ...
  return (
    <section className="screen-section" aria-labelledby="library-title">
      ...
      <label className="search-label"><span>Search</span><input type="search" value={query} placeholder="Number, title, path" onChange={(event) => setQuery(event.target.value)} /></label>
      ...
      <ul className="score-list" aria-live="polite">
        {filtered.map((score) => <li key={score.id}><button type="button" className="score-row" onClick={() => { setEditing(score.id); onEditingChange?.(true) }}><span className="score-number">{score.displayNumber}</span><span className="score-copy"><strong>{score.title}</strong><small>{score.availability === 'missing' ? 'Missing' : score.configuration === 'pending' ? 'Setup needed' : !score.enabled ? 'Disabled' : score.tags.includes('80s') ? 'Ready · 80s' : 'Ready'}</small></span><span className={`status-dot ${score.availability} ${score.configuration}`} aria-hidden="true" /></button></li>)}
      </ul>
      {filtered.length === 0 && <p className="notice">No matches.</p>}
    </section>
  )
}
